...
namespace RVO
{
    ...
    internal class KdTree
    {
         ...
         */
        private const int MAxLEAF_SIZE = 10;
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            if (end - begin > MAxLEAF_SIZE)
            {
                ...
                float splitValue = 0.5f * (isVertical ? agentTree_[node].maxx + agentTree_[node].minx : agentTree_[node].maxy + agentTree_[node].miny);
        ...
        {
            if (agentTree_[node].end_ - agentTree_[node].begin_ <= MAxLEAF_SIZE)
            {
            ...
            {
                float distSqLeft = RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].left_].minx - position.x)) + RVOMath.sqr(Math.Max(0.0f, position.x - agentTree_[agentTree_[node].left_].maxx)) + RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].left_].miny - position.y)) + RVOMath.sqr(Math.Max(0.0f, position.y - agentTree_[agentTree_[node].left_].maxy));
                float distSqRight = RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].right_].minx - position.x)) + RVOMath.sqr(Math.Max(0.0f, position.x - agentTree_[agentTree_[node].right_].maxx)) + RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].right_].miny - position.y)) + RVOMath.sqr(Math.Max(0.0f, position.y - agentTree_[agentTree_[node].right_].maxy));
        ...
        private void queryAgentTreeRecursive(Agent agent, ref float rangeSq, int node)
        {
            ...
            {
                float distSqLeft = RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].left_].minx - agent.position_.x)) + RVOMath.sqr(Math.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].left_].maxx)) + RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].left_].miny - agent.position_.y)) + RVOMath.sqr(Math.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].left_].maxy));
                float distSqRight = RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].right_].minx - agent.position_.x)) + RVOMath.sqr(Math.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].right_].maxx)) + RVOMath.sqr(Math.Max(0.0f, agentTree_[agentTree_[node].right_].miny - agent.position_.y)) + RVOMath.sqr(Math.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].right_].maxy));
